...
<portlet:actionURL name="filtroPermisos" var="filtroPermisos"></portlet:actionURL>
<portlet:resourceURL var="exportarExcel">
	<portlet:param name="action" value="permisosbyUserToExcel" />
</portlet:resourceURL>

<%
	Date hoy = new Date();
	int quarter = (hoy.getMonth() / 3) + 1;
	...
	if(login != null){
		List<licencia_permiso> permisosAux = (List<licencia_permiso>)request.getAttribute("permisos");
		if(permisosAux == null)
			permisosAux = licencia_permisoLocalServiceUtil.getByCreator(Long.parseLong(request.getRemoteUser()));
			List<licencia_permiso> permisos = new ArrayList<licencia_permiso>();
			for(licencia_permiso p : permisosAux){
...
<div style="display: inline-block; vertical-align: top; width: 82%;">
	...
	<liferay-ui:success key="vacio" message="No ha registrado ningún permiso" />
	<liferay-ui:success key="correctoExcelP" message="El archivo Excel de permisos se a cargado satisfactoriamente" />
	<div class="container">
		<div class="row">
			<form method="post" action="${filtroPermisos}">
				<div class="col-lg-3">
					<div class="col-lg-12">
				...
				<div class="col-lg-3">
					...
					<div class="col-lg-12">
						<select class="select-option" name="tipo">
		...
		<div class="row">
			<div class="col-md-12">
				<strong>NOTA:</strong> los permisos cancelados se mostraran resaltados en <strong style="color: red;">rojo</strong>
			</div>
			<div class="col-md-12">
				<liferay-ui:search-container   delta="10" deltaConfigurable="true"  emptyResultsMessage="No hay Resultados" iteratorURL="<%=iteratorURL%>">
					...
					<liferay-ui:search-container-row className="co.com.csj.model.licencia_permiso" keyProperty="id" modelVar="permiso">
						<%
							...
							usuario Usuario = usuarioLocalServiceUtil.getusuario(permiso.getUsuariofk());
							String userName = Usuario.getNomb() + " " + Usuario.getApe();
						...
						%>
							<% if(permiso.getEstatus()==0){ %>
								...
								<%-- <%
									int aux = (permiso.getFechafin().getMonth() / 3) + 1;
									if(permiso.getEstatus()==1){
									if(quarter == aux  && hoy.getYear()==permiso.getFechafin().getYear()){
								%>
								<liferay-ui:search-container-column-jsp  name="Accion"	path="/html/servidoresjudiciales/accionRegistro.jsp"></liferay-ui:search-container-column-jsp>
								<%
									}else{
								%>
									<liferay-ui:search-container-column-text name="Accion" value="Fuera del trimestre actual" />
								<%
									}
									}else{
								%>
									<liferay-ui:search-container-column-text name="Accion" value="Permiso Cancelado" />
								<%
									}
								%> --%>
							<%}else{ %>
...
<%=consultas.getContenidoWeb() %>
<%} %>
<script type="text/javascript">
$(document).ready( function(){
	$("#menu-permisosUsuario").addClass("active");
});
