...
<%
	Date hoy = new Date();
	int semestreActual = (hoy.getMonth()/6)+1;
	if(renderRequest.getPortletSession().getAttribute("semestreD")!=null){
		String a = (String) renderRequest.getPortletSession().getAttribute("semestreD");
		semestreActual=Integer.parseInt(a);
	}
	List<User> noRegistraron = consultas.getUsuariosNoRegistranD(semestreActual);
...
<portlet:resourceURL var="Exportar">
	<portlet:param name="action" value="exportarNoReportanD"/>
	<portlet:param name="semestre" value="<%=String.valueOf(semestreActual) %>"/>
</portlet:resourceURL>

<portlet:resourceURL var="alerta">
	<portlet:param name="action" value="alertaGuardarD" />
</portlet:resourceURL>

<portlet:actionURL var="enviarAlertaD" name="enviarAlertaD">
	<portlet:param name="semestre" value="<%=String.valueOf(semestreActual) %>"/>
</portlet:actionURL>

<portlet:actionURL var="semestreActualD" name="semestreActualD">
</portlet:actionURL>

<portlet:actionURL var="semestreAnteriorD" name="semestreAnteriorD">
	<portlet:param name="semestre" value="<%=String.valueOf(semestreActual-1) %>"/>
</portlet:actionURL>
	...
	<div class="content">
		<div class="row">
			<div class="col-lg-12" style="text-align: center;">
				<%if(renderRequest.getPortletSession().getAttribute("semestreD")==null){ %>
					<h1 style="font-size: 3em; margin: 0 0;">Semestre en Curso</h1>
					<%if(semestreActual==1){ %>
						<h1 style="color: red;">Enero - Junio</h1>
					<% } %>
					<%if(semestreActual==2){ %>
						<h1 style="color: red;">Julio - Diciembre</h1>
					<% } %>
					<a style="background: #19325b;color: #fff; border-radius: 0;" class="btn" href="${semestreAnteriorD}">Semestre Anterior</a>
					<br>
					<br>
				<%}else{ %>
					<h1 style="font-size: 3em; margin: 0 0;">Semestre Anterior</h1>
					<%if(semestreActual==1){ %>
						<h1 style="color: red;">Enero - Junio</h1>
					<% } %>
					<%if(semestreActual==2){ %>
						<h1 style="color: red;">Julio - Diciembre</h1>
					<% } %>
					<a style="background: #19325b;color: #fff; border-radius: 0;" class="btn" href="${semestreActualD}">Semestre Actual</a>
					<br>
					<br>
				<%} %>
			</div>
		...
		<div class="row">
			<div class="col-md-12">
				<h2 style="color:red;">Usuarios que no han registrado docencias e investigaciones en el semestre en curso</h2>
			</div>
			<div class="col-md-12">
